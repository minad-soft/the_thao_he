...
export async function POST(request: NextRequest) {
  ...
  // Validation
  if (!student_id && (!full_name || !dob || !gender || !phone_number || !school_id)) {
    return NextResponse.json(
      { error: "Thiếu trường bắt buộc thông tin học viên" },
      { status: 400 }
    );
  }
  
  if (!package_id || !receipt_number) {
    return NextResponse.json(
      { error: "Thiếu trường bắt buộc: package_id, receipt_number" },
      { status: 400 }
    );
  }

  // Validate gender if creating new student
  if (!student_id && gender !== "Nam" && gender !== "Nữ" && gender !== "Khác") {
    return NextResponse.json(
  ...
  if (!student_id && !phoneRegex.test(phone_number)) {
    return NextResponse.json(
  ...
  // 1. Truy xuất school_code nếu cần tạo mã thẻ mới
  let schoolCodeToUse = "";
  if (!student_id) {
    ...
    schoolCodeToUse = school.school_code;
  ...
  // 3. Xử lý học viên và mã thẻ
  let cardCode = "";
  let finalStudentId = student_id;

  if (student_id) {
    // Đã có học viên: Tìm mã thẻ cũ
    ...
    if (existingReg && existingReg.card_code) {
      cardCode = existingReg.card_code;
    } else {
      return NextResponse.json({ error: "Không tìm thấy mã thẻ của học viên này để dùng chung" }, { status: 400 });
    }
  } else {
    // 3a. Sinh mã thẻ mới với collision check (retry tối đa 5 lần)
    let attempts = 0;
    const maxAttempts = 5;

    while (attempts < maxAttempts) {
      cardCode = generateCardCode(schoolCodeToUse);
  ...
  if (regErr || !registration) {
    // Rollback: xóa student đã tạo (nếu tạo mới)
    if (!student_id) {
      await supabaseAdmin.from("students").delete().eq("id", finalStudentId);
    }
    return NextResponse.json(
  ...
  if (totalPaid > 0 && payments && payments.length > 0) {
    ...
    if (payErr) {
      // Rollback: xóa registration và student đã tạo (nếu có)
